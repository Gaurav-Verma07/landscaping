...
create policy "design_assets_plant_icons_update"
on "storage"."objects"
as permissive
for update
to public
using (((bucket_id = 'design-assets'::text) AND (auth.uid() IS NOT NULL) AND ((name ~~ 'plant-icons/%'::text) OR (name ~~ 'plant-thumbnails/%'::text))));


create policy "design_assets_plant_icons_write"
on "storage"."objects"
as permissive
for insert
to public
with check (((bucket_id = 'design-assets'::text) AND (auth.uid() IS NOT NULL) AND ((name ~~ 'plant-icons/%'::text) OR (name ~~ 'plant-thumbnails/%'::text))));
