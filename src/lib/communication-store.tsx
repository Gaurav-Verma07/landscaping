...
const CommunicationStoreContext = createContext<CommunicationStoreValue | null>(
  null,
);
...
export function CommunicationStoreProvider({
  ...
  const addRule = useCallback((r: Omit<AutomationRule, "id" | "createdAt">) => {
    const now = new Date().toISOString();
  ...
  const updateSequence = useCallback(
    (id: string, data: Partial<FollowUpSequence>) => {
      setSequences((prev) => {
        const next = prev.map((x) => (x.id === id ? { ...x, ...data } : x));
        localStorage.setItem(SEQUENCES_STORAGE_KEY, JSON.stringify(next));
        return next;
  ...
  const updateScheduledStatus = useCallback(
    (id: string, status: ScheduledMessage["status"]) => {
      setScheduledMessages((prev) => {
  ...
  const triggerAutomation = useCallback(
    (
      ...
      relevantRules.forEach((rule) => {
        const baseDelayMs = (rule.delayDays ?? 0) * dayMs;
        const sendAt = new Date(now + baseDelayMs).toISOString();
        addScheduledMessage({
