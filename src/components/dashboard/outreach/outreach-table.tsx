...
export function OutreachTable({
  ...
  const getStageBadgeVariant = (stage: OutreachStage) => {
    if (stage === 'Partner') return 'default'
    if (stage === 'Qualified') return 'default'
      ...
      <Table>
        ...
        <TableBody>
          {paged.map((p) => (
            <TableRow
              key={p.id}
              className={selectedIds.has(p.id) ? 'bg-muted/50' : ''}
            >
              <TableCell>
                <Checkbox
              ...
              <TableCell>
                <Badge variant={getStageBadgeVariant(p.stage)} className="text-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            <Button variant="outline" size="icon" className="h-8 w-8"
              onClick={() => onPageChange(0)} disabled={pageIndex === 0}>
              <IconChevronsLeft className="h-4 w-4" />
            </Button>
            <Button variant="outline" size="icon" className="h-8 w-8"
              onClick={() => onPageChange(Math.max(0, pageIndex - 1))} disabled={pageIndex === 0}>
            ...
            <Button variant="outline" size="icon" className="h-8 w-8"
              onClick={() => onPageChange(pageCount - 1)} disabled={pageIndex >= pageCount - 1}>
