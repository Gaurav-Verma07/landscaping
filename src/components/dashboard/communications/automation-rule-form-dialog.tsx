...
import type { AutomationRule, AutomationTrigger } from "@/lib/communication-types"
...
export function AutomationRuleFormDialog({
  ...
  const handleSubmit = () => {
    ...
    onSave({ name: name.trim(), trigger, delayDays, templateId, enabled: true })
    onOpenChange(false)
    setName("")
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            When a trigger happens, schedule a message after a delay using a template.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          <FieldGroup>
            <FieldLabel>Name</FieldLabel>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Quote follow-up" />
          </FieldGroup>
          <FieldGroup>
